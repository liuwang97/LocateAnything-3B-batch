...
def generate_batch(pairs, temperature=0.0, top_p=None, top_k=None,
                   repetition_penalty=1.0, max_new_tokens=MNT):
...
def _decode(prompt_ids, L, kv, kvlen, kvalid,
            temperature, top_p, top_k, repetition_penalty, max_new_tokens, row_temps=None):
    """Shared batched fast-MTP decode. Prefill is already done: kv = legacy-tuple KV of the
    full prompts, kvlen = KV column count, kvalid = [B,kvlen] column-validity, L[b] = row b's
    REAL prompt length, prompt_ids[b] = row b's full real token ids (for repetition penalty).
    row_temps (optional, len B) = per-row temperatures; None -> scalar `temperature` for all.
    Used by both generate_batch (one-shot prefill) and generate_batch_grouped (prefix reuse)."""
    tok, proc, model = load()
